...
agentDesignationLabelsToKeep?: string[] | null # Optional: List of agent label names for which the label is NOT removed after the Agent project field is set. When an issue carries a label matching an agent name in this list, the Agent field is updated normally but the label is kept on the issue (the normal behavior removes it). Use this for a label such as `story` that serves a dual purpose and must remain visible on the issue after routing. Declared at the top level (sibling of startPreparation), not inside it.
...
fallbackLlmModelName?: string # Optional: LLM model used when defaultLlmModelName is not specified (default: claude-opus-4-8). Per-issue llm-model: labels remain authoritative
...
thresholdForAutoReject?: number # Optional: Consecutive rejections before escalation (default: 3). Applies to orphaned-preparation, Auto Status Check reject, and conflict-revert paths alike
thresholdForDispatchLoop?: number # Optional: How many times one agent may be dispatched on a single issue since the last human comment before the issue is moved to Failed Preparation instead of being dispatched again (default: 6). This bounds dispatch loops in which the agents do report each round, both when an agent names itself as the next step and when two agents name each other in turn. Any comment that is neither an agent report nor a comment this tool posts itself counts as human input and restarts the count. Applies to orphaned-preparation, Auto Status Check reject, and conflict-revert paths alike
workflowBlockerResolvedWebhookUrl?: string # Optional: Webhook URL. Supports {URL} and {MESSAGE} placeholders
preparationProcessCheckCommand?: string # Optional: Shell command template with {URL} placeholder to check if a preparation process is alive. Orphaned Preparation issues (process exits non-zero, or stale aw log) are evaluated for completion: if work is done they advance to Awaiting Owner; otherwise an `Auto Status Check: REJECTED` comment is posted and the issue falls back to Awaiting Workspace, except that once `thresholdForAutoReject` cumulative orphan-time rejections accumulate within the recent comment window (and no earlier escalation marker is present) the issue is transitioned to Failed Preparation instead.
...
consoleGithubTokens?: Record<string, string> # Optional: Inline map of pjcode to GitHub token. When set together with consoleProjects, serveWeb resolves a per-project GH token by matching the repository owner against each project URL. When a matching pjcode is found but its token is absent or blank in the map, an error is thrown. Falls back to the default token when consoleGithubTokens is unset or when no pjcode maps to the owner.
...
silentNotificationEnabled?: boolean # Optional: Master switch for the silent live session self-check notification step. The step is a no-op unless this is `true`, the fleet-wide config file named by `--fleetConfigFilePath` (or `TDPM_FLEET_CONFIG`) contains a top-level `silentNotificationEnabled: true`, or the `TDPM_SILENT_NOTIFICATION_ENABLED` environment variable equals the string `true`. Priority order: this per-project config key (highest), fleet config (middle), env var (lowest). Default false
...
errorReportingRepository?: string # Optional: GitHub repository in `owner/repo` format for TDPM-level issue routing. When set: (1) any unhandled error that escapes the CLI process creates a new GitHub issue here (title: `CLI error: {errorName}: {first 80 chars of message}`), or adds a comment to an existing open issue with the same title; (2) "Register missing agent definition" tasks (triggered when an agent definition file is absent) and "Unregistered agent in workflow configuration" blocker issues are created here instead of the product repository. Requires GH_TOKEN. Can also be set via the TDPM_ERROR_REPORT_REPOSITORY environment variable without a config file.
```

Workflow status names (`Unread`, `Awaiting Workspace`, `Awaiting Owner`, `Preparation`, `Failed Preparation`, `Todo by human`, `In Tmux by human`, `In Tmux by agent`, `Done`, `Icebox`) are hardcoded constants and are not accepted via this config, the CLI, or the project README. To ensure they exist on the target project, run the `schedule` command — it invokes `SetupTowerDefenceProjectUseCase` automatically. Projects with the legacy `Todo` and `In Tmux` status names are automatically migrated; `PC Todo` and `Awaiting Task Breakdown` are removed on the next setup run (items in `Awaiting Task Breakdown` are moved to `Todo by human` before the option is deleted). `Awaiting Owner` is a required workflow status created automatically when absent.
...
```json
{
  ...
  "status": {
    "awaitingOwnerImmediatelyActionable": 2,
    "preparation": 4,
...
- `status.awaitingOwnerImmediatelyActionable`: Count of issues in the awaiting owner status with no dependency URL, next action date, or next action hour set.
...
- `prs`: status equals `Awaiting Owner` (case-insensitive)
...
Each schedule cycle, when `silentNotificationEnabled` is true, this step runs and logs that the notification step is active but sends no notifications. When `silentNotificationEnabled` is not set, the step logs that it is skipped. Any error during the step is logged and swallowed so the schedule cycle is never affected.

`TDPM_SILENT_NOTIFICATION_ENABLED` (the string `true` enables the step) is the environment variable equivalent. The fleet-wide config file's top-level `silentNotificationEnabled` is the intermediate source between the per-project config key and this env var.
...
- `RevertNotReadyReviewQueueIssueUseCase` — handles two review-queue statuses in a single pass:
  - Awaiting Owner items: reverts items whose linked PR is conflicting, draft, missing, multiplied, has failing or never-started CI checks, or has unresolved review comments back to awaiting-workspace. For items that have no rejections it also runs the `change-target:<path>` auto-approve check: when the item carries one or more `change-target:` labels and every changed file of the ready PR is confined to the labeled paths, the PR is auto-approved.
...
- Clearing or rewriting the Depended Issue URL field through the repository replaces the matching issue in the memoized issue list and writes the new value into the cache file, so every later read in the same process (`getAllIssues`, `getStoryObjectMap`, `getAllOpened`, `getIssueByUrl`) sees the written value without another GitHub call. Issue objects handed out before the write keep the value they had.
...
- Because the memoized list can be older than a value another client wrote a few seconds ago, the use cases that pick an item from that list and write its Story, Status or Agent first read that single item live through `get` (one item query, no list pagination) immediately before the write. The write is skipped, with a warning naming the changed field and both values, when a field that made the item eligible no longer holds its listed value or the item is no longer on the project; the next cycle evaluates the item again from fresh data. This applies to `SetNoStoryIssueToStoryUseCase` and `SetWorkflowManagementIssueToStoryUseCase` (Story), the default Agent write of `AgentDesignationLabelAdoptUseCase`, and the Status writes of `IssueNoStatusUpdateUseCase`, `ChangeStatusByStoryColorUseCase`, `QualityCheckAdvanceUseCase`, `ReopenedDoneIssueRevertUseCase`, `RevertNotReadyReviewQueueIssueUseCase`, the open-item recovery of `SetupTowerDefenceProjectUseCase`, and the Todo by human Status write for an author-not-allowed issue in `StartPreparationUseCase`. `StartPreparationUseCase` re-reads each spawn candidate live through `get` rather than from the on-disk cache before moving it to Preparation.
