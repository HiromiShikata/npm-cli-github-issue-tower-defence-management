...
tokenExhaustionHandoverEnabled?: boolean # Optional: When true, the daemon detects interactive sessions whose OAuth token has exceeded the rate-limit exhaustion threshold (fiveHourUtilization >= 0.9, sevenDayUtilization >= 0.95, blocked, rejected, or blockedUntilEpoch in the future) and sends a configurable handover message into the tmux session via `tmux send-keys`, then kills the session after the grace period so the reconciler can restart it with a fresh token. Requires `claudeCodeOauthTokenListJsonPath`. When false, detection and logging still run but no `send-keys` or kill is performed (dry-run). Default false
tokenExhaustionHandoverMessage?: string # Optional: Text sent into the exhausted session via `tmux send-keys` when token exhaustion is detected. When unset, a built-in default message is used
...
subAgentRuntimeRootDirectory?: string # Optional: Runtime root directory (for example `/tmp/claude-<uid>`) under which the harness writes each session's positive running-sub-agent set as `<cwd-slug>/<sessionId>/running-subagents.txt`, one live sub-agent id per line. When transcript-based discovery is used, this enables a positive process-liveness gate: a sub-agent whose id is absent from the running set has already exited, so its stale on-disk transcript is not flagged even when its tail is a plain in-flight `tool_result`. When this directory is unset it defaults to `<os-tmpdir>/claude-<uid>` (or is disabled on platforms without a numeric user id); when the directory or the running-set file cannot be read the gate is skipped (fail-open) and the terminal-status and completion-marker exclusions remain the sole guards
