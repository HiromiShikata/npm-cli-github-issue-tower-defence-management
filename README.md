...
Options for startDaemon:
  ...
  --fallbackLlmModelName <name>                    LLM model to fall back to when the default Sonnet model is selected but its 7-day weekly limit is exhausted across all tokens (default: claude-opus-4-5)
...
startPreparation?: # Optional: Enable automatic issue preparation workflow
  ...
  fallbackLlmModelName?: string | null # Optional: LLM model to fall back to when defaultLlmModelName is a Sonnet model and its 7-day weekly limit is exhausted across all tokens while the fallback model still has capacity (default: claude-opus-4-5). Per-issue llm-model: labels remain authoritative and are never overridden by the fallback
...
fallbackLlmModelName?: string # Optional: LLM model to fall back to when defaultLlmModelName is a Sonnet model and its 7-day weekly limit is exhausted across all tokens while the fallback model still has capacity (default: claude-opus-4-5). Per-issue llm-model: labels remain authoritative
