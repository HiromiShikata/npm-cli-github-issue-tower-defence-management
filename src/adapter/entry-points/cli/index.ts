...
program
  ...
  .description(
    'Print exactly one Claude Code OAuth token chosen for a new live interactive session. The choice is deterministic. Each rate-limit-eligible token gets a concurrent session limit of maxConcurrentSessionCount (default 10), scaled by its per-token selectionWeight (default 1). That limit is held at full value while the free share of the 5h window is at or above fullSpeedFiveHourFreeRatio (default 0.5) and is tapered in proportion below it, and it is further capped at the number of sessions the free 5h share sustains until the 5h window resets, at fiveHourShareConsumedPerSessionHour (default 0.05) of the window per session-hour and never more than a full window spread over 5 hours carries; it never drops under 1. The 7d window never lowers the limit, so a weekly allowance that is about to expire is drained as fast as the 5h window allows instead of being discarded unused. Among eligible tokens still under that limit the token whose 7d window resets soonest wins; ties go to the token carrying fewer live sessions (every running Claude Code process holding the token counts, keyed by CLAUDE_CONFIG_DIR, else CLAUDE_CODE_SESSION_ID, else its process id). When every eligible token is at its limit the token least over its limit after one more session is returned. The token string is written to stdout (pipeable); the per-candidate decision trace is written to stderr. Exits non-zero when no token passes the filter.',
  )
  .option(
    '--tokenListJsonPath <path>',
