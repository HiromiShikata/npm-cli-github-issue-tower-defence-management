...
Options for notifyFinishedIssuePreparation:
  ...
  --thresholdForSelfNominationTotal <count>        Threshold for auto-escalation when an agent self-nominates this many times across the full issue history, ignoring human-comment cycle resets (default: 2)
...
thresholdForSelfNominationTotal?: number # Optional: How many times an agent may self-nominate across the full issue history (ignoring human-comment cycle resets) before the issue is moved to Failed Preparation (default: 2). This bounds loops where the owner posts human comments between cycles, which would otherwise reset the cycle-based counters indefinitely
