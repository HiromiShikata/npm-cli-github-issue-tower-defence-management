...
codexHomeCandidates:
  ...
  - .codex-main
    ...
    it('should not warn when only known keys are present', () => {
      const readme = `<details>
<summary>config</summary>
defaultAgentName: 'known-agent'
maximumPreparingIssuesCount: 10
</details>`;
      const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation();
      ...
      expect(result.defaultAgentName).toBe('known-agent');
      expect(result.maximumPreparingIssuesCount).toBe(10);
      expect(consoleWarnSpy).not.toHaveBeenCalled();

      consoleWarnSpy.mockRestore();
    });

    it('should warn for each unknown key and include the project URL', () => {
      const readme = `<details>
<summary>config</summary>
unknownOne: 'value1'
unknownTwo: 42
      ...
      expect(result).toEqual({
        ...
        defaultLlmAgentName: undefined,
        maximumPreparingIssuesCount: undefined,
        allowIssueCacheMinutes: undefined,
        utilizationPercentageThreshold: undefined,
        allowedIssueAuthors: undefined,
        thresholdForAutoReject: undefined,
        workflowBlockerResolvedWebhookUrl: undefined,
        preparationProcessCheckCommand: undefined,
        codexHomeCandidates: undefined,
        claudeCodeOauthTokenListJsonPath: undefined,
        awLogDirectoryPath: undefined,
        awLogStaleThresholdMinutes: undefined,
      });
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Unknown key "unknownOne" in project README config section (project: https://github.com/orgs/test/projects/1)',
      );
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Unknown key "unknownTwo" in project README config section (project: https://github.com/orgs/test/projects/1)',
      );

      consoleWarnSpy.mockRestore();
    });

    it('should warn for unknown keys in mixed known and unknown config', () => {
      const readme = `<details>
<summary>config</summary>
defaultAgentName: 'mixed-agent'
unknownExtra: 'extra-value'
</details>`;
      const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation();
      ...
      expect(result.defaultAgentName).toBe('mixed-agent');
      expect(consoleWarnSpy).toHaveBeenCalledTimes(1);
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Unknown key "unknownExtra" in project README config section (project: https://github.com/orgs/test/projects/2)',
      );

      consoleWarnSpy.mockRestore();
    });

    it('should warn when a known key is misspelled (typo of known)', () => {
      const readme = `<details>
<summary>config</summary>
maximumPreparingIssueCount: 12
</details>`;
      const consoleWarnSpy = jest.spyOn(console, 'warn').mockImplementation();

      const result = parseProjectReadmeConfig(
        readme,
        'https://github.com/orgs/test/projects/3',
      );

      expect(result.maximumPreparingIssuesCount).toBeUndefined();
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Unknown key "maximumPreparingIssueCount" in project README config section (project: https://github.com/orgs/test/projects/3)',
      );

      consoleWarnSpy.mockRestore();
    });

    it('should omit project URL from warning when not provided', () => {
      const readme = `<details>
<summary>config</summary>
unknownKey: 'value'
</details>`;
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'Unknown key "unknownKey" in project README config section',
      );
