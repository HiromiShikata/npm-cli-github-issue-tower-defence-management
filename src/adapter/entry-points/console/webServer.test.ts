...
describe('webServer new routes integration', () => {
  ...
  it('calls consoleErrorReporter with error and request path when an operation rejects', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'console-server-'));
    const issueRepository = mock<IssueRepository>();
    issueRepository.get.mockResolvedValue({
      ...mock<Issue>(),
...
describe('webServer GET /api/projects', () => {
  ...
  it('GET /api/features always returns { airplaneMode: true }', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'console-server-'));
    const server = await startWebServer({
      accessToken: testToken,
      uiDistDir: path.join(tmpDir, 'ui-dist'),
  ...
  it('GET /api/airplanesync returns 404 when issueRepository is not configured', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'console-server-'));
    const server = await startWebServer({
      accessToken: testToken,
      uiDistDir: path.join(tmpDir, 'ui-dist'),
  ...
  it('POST /api/airplanesync returns 400 when targetUrls is not an array', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'console-server-'));
    const server = await startWebServer({
      accessToken: testToken,
      uiDistDir: path.join(tmpDir, 'ui-dist'),
  ...
  it('POST /api/airplanesync returns 404 when issueRepository is not configured', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'console-server-'));
    const server = await startWebServer({
      accessToken: testToken,
      uiDistDir: path.join(tmpDir, 'ui-dist'),
    ...
    try {
      const response = await request(
        ...
        'POST',
        `/api/airplanesync?k=${testToken}`,
        { targetUrls: ['https://github.com/o/r/issues/1'] },
      );
      expect(response.statusCode).toBe(404);
    } finally {
      await closeServer(server);
