...
for `tab` in `workflow-blocker`, `prs`, `triage`, `unread`, `failed-preparation`, `todo-by-human`, `todo-by-agent`, and `in-tmux-by-agent`. Each file is written atomically (written to a `.tmp` file then renamed) so external readers never see a partial write. When `consoleDataOutputDir` is unset the generation is skipped, and any error during generation is logged and swallowed so the schedule cycle is never affected.
...
- `in-tmux-by-agent`: status equals `In Tmux by agent` (case-insensitive); lists all open issues assigned to the project manager regardless of actionability, so issues with a next action date, next action hour, or depended issue URLs are included
- `triage`: every open issue assigned to the console owner whose story field is empty or whose story name contains `no story` (case-insensitive); pull requests and `In Tmux by agent` issues are excluded; the issue's status, next action date, next action hour, and depended issue URLs do not affect inclusion, so the triage decision stays open however far the status has advanced and however the issue is blocked or scheduled
...
The `workflow-blocker`, `prs`, `unread`, `failed-preparation`, `todo-by-human`, `todo-by-agent`, and `in-tmux-by-agent` tabs share this shape:
...
- `storyColors`: Map from story name to its color. Object value (`{ "color": ... }`) for `workflow-blocker`/`prs`/`unread`/`failed-preparation`/`todo-by-human`/`todo-by-agent`/`in-tmux-by-agent`; plain string value for `triage`.
