...
The `writeAgentHeartbeat` sub-command writes a heartbeat timestamp for the given issue URL to a file in `--heartbeatDir` (default: `/tmp/tdpm-agent-heartbeats`). An agent working on an issue from a workspace-named tmux session (for example `tdpm-cli`) must call this command periodically — no less often than every `minOrphanAgeSeconds / 2` seconds (default: every 150 s, i.e. well under the 300 s window) — so that the periodic orphan-revert sweep (`RevertOrphanedInTmuxByAgentIssueUseCase`) recognises the issue as actively held and does not move it back to `Awaiting Workspace`. For issues whose agent runs in a session named after the issue URL (the common subagent case), the sweep automatically refreshes the heartbeat on every cycle while that session is alive, so no explicit call is needed for those. For workspace-leader sessions the calling agent is responsible for the periodic refresh. The sweep considers a heartbeat fresh when it was written within `minOrphanAgeSeconds` of the sweep time; a stale or absent heartbeat together with no issue-named tmux session and no matching process command line causes the issue to be reverted. The heartbeat file is named by replacing non-alphanumeric characters in the issue URL (after stripping the scheme) with `-`, so two agents working different issues never collide. The command writes nothing to stdout on success.
