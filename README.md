...
The `writeAgentHeartbeat` sub-command writes a heartbeat timestamp for the given issue URL to a file in `--heartbeatDir` (default: `/tmp/tdpm-agent-heartbeats`). An agent working on an issue from a workspace-named tmux session (for example `tdpm-cli`) calls this command once when it starts working so that the periodic orphan-revert sweep (`RevertOrphanedInTmuxByAgentIssueUseCase`) recognises the issue as actively held and does not move it back to `Awaiting Workspace` before the agent finishes. The sweep considers a heartbeat fresh when it was written within `minOrphanAgeSeconds` (default 300) of the sweep time; a stale or absent heartbeat together with no issue-named tmux session and no matching process command line causes the issue to be reverted. The heartbeat file is named by replacing non-alphanumeric characters in the issue URL (after stripping the scheme) with `-`, so two agents working different issues never collide. The command writes nothing to stdout on success.
