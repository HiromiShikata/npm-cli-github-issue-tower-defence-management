...
awaitingQualityCheckViewerOutputPath?: string # Optional: Absolute path to write a PR-review viewer JSON file after the quality-check run. When set, issues that are in Awaiting Quality Check status, assigned to HiromiShikata, have no Next Action Date or Next Action Hour, and have exactly one ready open PR are collected and written as JSON to this path (atomic write via .tmp rename). The JSON contains a `stories` array (with hex color codes) and an `items` array with issue, PR, and changedDirectories fields
awaitingQualityCheckDoneStorePath?: string # Optional: Absolute path to a JSON file maintained by a local PR-review server that records already-reviewed PRs. When set, PRs whose URL appears in this file are excluded from the viewer output produced by awaitingQualityCheckViewerOutputPath. The file may contain an array of URL strings or an array of objects with a `url` field. If the file is absent or malformed, it is silently ignored and all qualifying PRs are included
```
...
- `RevertNotReadyAwaitingQualityCheckUseCase` — reverts awaiting-quality-check issues whose linked PR is conflicting, draft, missing, multiplied, has failing or never-started CI checks, or has unresolved review comments, back to awaiting-workspace. For awaiting-quality-check issues that have no rejections, it also runs the same `change-target:<path>` auto-approve check as `NotifyFinishedIssuePreparationUseCase`: when the issue carries one or more `change-target:` labels and every changed file of the ready PR is confined to the labeled paths, the PR is auto-approved (issues with an `llm-agent` label are skipped).
