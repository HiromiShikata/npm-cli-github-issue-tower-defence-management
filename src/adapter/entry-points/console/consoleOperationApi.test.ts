...
describe("consoleOperationApi", () => {
	...
	describe("handleStoryAdd", () => {
		...
		it("invalidates the project cache after each successful add so consecutive adds both survive", async () => {
			...
			const secondCallStories = consecutiveUpdateStoryList.mock.calls[1][1] as {
				name: string;
			}[];
			expect(secondCallStories.some((s) => s.name === "Story X")).toBe(true);
			expect(secondCallStories.some((s) => s.name === "Story Y")).toBe(true);
		});
	});
});
