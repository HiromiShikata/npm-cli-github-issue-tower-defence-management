...
test("immediately shows item in Queued tab after moving to Awaiting Workspace", async ({
	page,
}) => {
	await page.goto(harness.appUrl);
	...
	const queuedListPath = path.join(
		harness.consoleDataOutputDir,
		CONSOLE_E2E_PJCODE,
		"queued",
		"list.json",
	);
	const prsListPath = path.join(
		harness.consoleDataOutputDir,
		CONSOLE_E2E_PJCODE,
		"prs",
		"list.json",
	...
	const queuedSnapshot = JSON.parse(
		fs.readFileSync(queuedListPath, "utf8"),
	) as { items: unknown[]; generatedAt: string };
	const prsSnapshot = JSON.parse(fs.readFileSync(prsListPath, "utf8")) as {
		items: unknown[];
	};
	const movedItem = {
		...(prsSnapshot.items[0] as Record<string, unknown>),
	...
	fs.writeFileSync(
		...
		JSON.stringify({
			...
			items: [...queuedSnapshot.items, movedItem],
		}),
	);
