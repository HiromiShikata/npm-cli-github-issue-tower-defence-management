...
skipAgentAuthoredIssues?: boolean # Optional: When true, issues whose body starts with the agent report prefix (`From: :robot:`) are skipped by the preparation daemon and never auto-assigned to a workspace agent. Use this to prevent bot-created workflow improvement issues from filling the owner's merge approval queue. Default false. Declared at the top level (sibling of startPreparation), not inside it.
