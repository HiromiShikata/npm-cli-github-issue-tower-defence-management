...
describe('consoleOperationApi', () => {
  ...
  describe('handleTriage', () => {
    ...
    it('updates the in-memory cache with freshProject after stale-id retry without emptying it', async () => {
      const cachedStoryOption = {
        id: 'stale_opt',
        name: 'My story',
        color: 'BLUE' as const,
      ...
      issueRepository.updateStory
        .mockRejectedValueOnce(staleError)
        .mockResolvedValue(undefined);
      const loadProject = jest.fn().mockResolvedValue(cachedProject);
      ...
      const getProject = jest.fn().mockResolvedValue(freshProject);
      const contextWithRealResolver: ConsoleOperationContext = {
        ...context,
        resolveProject: resolver.resolve,
        invalidateProject: resolver.invalidate,
        updateProjectCacheEntry: resolver.updateEntry,
        resolveProjectRepository: () => ({
          getProject,
          updateStoryList: jest.fn(),
        }),
      ...
      const response = await handleTriage(contextWithRealResolver, {
        pjcode: 'acme',
        action: 'set_story',
        issueUrl: 'https://github.com/o/r/issues/1',
        projectItemId: 'PVTI_h',
      ...
      expect(response.statusCode).toBe(200);
      const cachedBinding = await resolver.resolve('acme');
      expect(cachedBinding?.project.story?.fieldId).toBe('fresh_field_id');
  ...
  describe('handleCreateIssue', () => {
    ...
    it('creates issue and runs background setup using cached data when resolveProjectRepository is null', async () => {
      const storyProject = projectWithStory();
      const createdIssue: Issue = {
        ...mock<Issue>(),
        url: 'https://github.com/acme-labs/portal/issues/42',
  ...
  describe('handleReorderStory', () => {
    ...
    describe('storyOptionId existence in cached vs. freshly-fetched data (issue #2603)', () => {
      ...
      const existenceCases: {
        name: string;
        cachedIds: string[];
        freshIds: string[];
        expectedStatusCode: number;
        expectReorderApplied: boolean;
      }[] = [
        {
          name: 'row 1: storyOptionId exists in both cached and fresh data -> succeeds',
          cachedIds: ['opt_x', 'opt_target', 'opt_y'],
          freshIds: ['opt_x', 'opt_target', 'opt_y'],
          expectedStatusCode: 200,
          expectReorderApplied: true,
        },
        {
          name: 'row 2: storyOptionId absent from cached data but present in fresh data (recently added/renamed story) -> succeeds',
          cachedIds: ['opt_x', 'opt_y'],
          freshIds: ['opt_x', 'opt_target', 'opt_y'],
          expectedStatusCode: 200,
          expectReorderApplied: true,
        },
        {
          name: 'row 3: storyOptionId present in cached data but absent from fresh data (recently deleted story) -> returns not-found error (already correct today)',
          cachedIds: ['opt_x', 'opt_target', 'opt_y'],
          freshIds: ['opt_x', 'opt_y'],
          expectedStatusCode: 400,
          expectReorderApplied: false,
        },
        {
          name: 'row 4: storyOptionId absent from both cached and fresh data -> returns not-found error',
          cachedIds: ['opt_x', 'opt_y'],
          freshIds: ['opt_x', 'opt_y'],
          expectedStatusCode: 400,
          expectReorderApplied: false,
      ...
      it.each(existenceCases)(
        '$name',
        async ({
          cachedIds,
          freshIds,
          expectedStatusCode,
          expectReorderApplied,
        }) => {
          const cachedProject: Project = {
            ...project,
            story: {
              ...
              fieldId: 'storyField',
              databaseId: 1,
              stories: cachedIds.map(buildStoryOption),
              workflowManagementStory: { id: 'wms', name: 'workflow' },
            },
          };
          const cachedStory = cachedProject.story;
          ...
          const freshProject: Project = {
            ...cachedProject,
            story: { ...cachedStory, stories: freshIds.map(buildStoryOption) },
          };
          const localUpdateStoryList = jest.fn().mockResolvedValue([]);
          ...
          const response = await handleReorderStory(
            ...
            { pjcode: 'acme', storyOptionId: 'opt_target', direction: 'up' },
          );
          expect(response.statusCode).toBe(expectedStatusCode);
          if (expectReorderApplied) {
      ...
      it('row 2b: storyOptionId absent from cached data but present in fresh data (recently added/renamed story), direction down -> succeeds', async () => {
        const cachedProject: Project = {
          ...project,
          story: {
            name: 'Story',
    ...
    describe('reorder boundary check uses the freshly-fetched list length (issue #2603)', () => {
      ...
      it('row 5: fresh list is longer than cached list because a story was appended after the cache snapshot was taken -> the newly possible move succeeds', async () => {
        const cachedProject: Project = {
          ...project,
          story: {
            name: 'Story',
        ...
        expect(localUpdateStoryList).toHaveBeenCalledWith(expect.anything(), [
          expect.objectContaining({ id: 'opt_a' }),
          expect.objectContaining({ id: 'opt_b' }),
          expect.objectContaining({ id: 'opt_d' }),
          expect.objectContaining({ id: 'opt_c' }),
        ]);
      });

      it('row 5b: fresh list is longer than cached list because a story was inserted before the cached window, direction up -> the newly possible move succeeds', async () => {
        const cachedProject: Project = {
          ...project,
          story: {
            name: 'Story',
        ...
        expect(localUpdateStoryList).toHaveBeenCalledWith(expect.anything(), [
          expect.objectContaining({ id: 'opt_a' }),
          expect.objectContaining({ id: 'opt_z' }),
          expect.objectContaining({ id: 'opt_b' }),
          expect.objectContaining({ id: 'opt_c' }),
        ]);
      });

      it('row 6: fresh and cached list lengths are unchanged -> the existing boundary rejection is unaffected', async () => {
        const cachedProject: Project = {
          ...project,
          story: {
            name: 'Story',
      ...
      it('row 6b: fresh and cached list lengths are unchanged, direction up -> the existing boundary rejection is unaffected', async () => {
        const cachedProject: Project = {
          ...project,
          story: {
            name: 'Story',
    ...
    describe('fresh project fetch fallback to the cached story list (issue #2619)', () => {
      it('reorders using the previously-cached story list when the fresh project fetch does not succeed', async () => {
        const cachedProject = projectWithOrderedStories();
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        ...
        expect(localUpdateStoryList).toHaveBeenCalledWith(
          expect.objectContaining({ id: cachedProject.id }),
          [
            expect.objectContaining({ id: 'opt_b' }),
            expect.objectContaining({ id: 'opt_a' }),
            expect.objectContaining({ id: 'opt_c' }),
    ...
    describe('mutating call must use the freshly-fetched story fieldId, not the stale cached one', () => {
      const buildOrderedProjectWithFieldId = (fieldId: string): Project => ({
        ...projectWithOrderedStories(),
        story: {
          name: 'Story',
          fieldId,
      ...
      it('calls updateStoryList with a project whose story.fieldId equals the freshly-fetched fieldId, not the stale cached fieldId', async () => {
        const cachedProject = buildOrderedProjectWithFieldId('PVTSSF_stale');
        const freshProject = buildOrderedProjectWithFieldId('PVTSSF_fresh');
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        const response = await handleReorderStory(
          contextWithProjectRepository(
            () => ({
  ...
  describe('handleStoryColor', () => {
    ...
    describe('project data written to the cache after a color change (issue #2619)', () => {
      it('writes the complete freshly-fetched project data to the cache when the fresh project fetch succeeds, not the cached project combined with only the refreshed story list', async () => {
        const cachedProject: Project = {
          ...projectWithStory(),
        ...
        const freshProject: Project = {
          ...cachedProject,
          name: 'Fresh Project Name From Server',
        };
        const updatedEntries: { pjcode: string; project: Project }[] = [];
        const ctx: ConsoleOperationContext = {
          ...contextForProject(cachedProject),
        ...
        const response = await handleStoryColor(ctx, {
          pjcode: 'acme',
          storyOptionId: 'opt_blue',
          newColor: 'RED',
          nameWithOwner: 'acme-labs/portal',
        ...
        expect(response.statusCode).toBe(200);
        expect(updatedEntries).toHaveLength(1);
        expect(updatedEntries[0].project.name).toBe(
          'Fresh Project Name From Server',
        );
      });

      it('combines the previously-cached project data with the resolved story list when the fresh project fetch does not succeed, matching current behavior', async () => {
        const cachedProject: Project = {
          ...projectWithStory(),
          name: 'Cached Project Name',
        };
    ...
    describe('storyOptionId existence in cached vs. freshly-fetched data (issue #2603)', () => {
      ...
      const existenceCases: {
        ...
        freshIds: string[];
        expectedStatusCode: number;
        expectColorChangeApplied: boolean;
      }[] = [
        {
          name: 'row 1: storyOptionId exists in both cached and fresh data -> succeeds',
          cachedIds: ['opt_other', 'opt_target'],
          freshIds: ['opt_other', 'opt_target'],
          expectedStatusCode: 200,
          expectColorChangeApplied: true,
        },
        {
          name: 'row 2: storyOptionId absent from cached data but present in fresh data (recently added/renamed story) -> succeeds',
          cachedIds: ['opt_other'],
        ...
        {
          name: 'row 3: storyOptionId present in cached data but absent from fresh data (recently deleted story) -> returns not-found error and does not change the color',
          cachedIds: ['opt_other', 'opt_target'],
          freshIds: ['opt_other'],
          expectedStatusCode: 400,
          expectColorChangeApplied: false,
        },
        {
          name: 'row 4: storyOptionId absent from both cached and fresh data -> returns not-found error',
          cachedIds: ['opt_other'],
    ...
    describe('mutating call must use the freshly-fetched story fieldId, not the stale cached one', () => {
      const buildProjectWithFieldId = (fieldId: string): Project => ({
        ...projectWithStory(),
        story: {
          name: 'Story',
          fieldId,
      ...
      it('calls updateStoryOptionColor with project.story.fieldId equal to the freshly-fetched fieldId, not the stale cached fieldId', async () => {
        const cachedProject = buildProjectWithFieldId('PVTSSF_stale');
        const freshProject = buildProjectWithFieldId('PVTSSF_fresh');
        const response = await handleStoryColor(
          {
            ...contextForProject(cachedProject),
            resolveProjectRepository: () => ({
  ...
  describe('handleDeleteStory', () => {
    ...
    it('clears the story field on open tasks when deleteChildTasks is false', async () => {
      const p = projectWithStoriesToDelete();
      const { story } = p;
      if (story === null) throw new Error('test fixture must have story');
      const storyToRemove = story.stories.find((s) => s.id === 'opt_remove');
      ...
      expect(issueRepository.closeIssueByUrl).not.toHaveBeenCalledWith(
        'https://github.com/acme-labs/ops/issues/100',
        expect.anything(),
      );
    });

    it('closes the story issue even when deleteChildTasks is false', async () => {
      const p = projectWithStoriesToDelete();
      const { story } = p;
      if (story === null) throw new Error('test fixture must have story');
      const storyToRemove = story.stories.find((s) => s.id === 'opt_remove');
    ...
    it('returns 200 even when clearProjectField throws when deleteChildTasks is false', async () => {
      const p = projectWithStoriesToDelete();
      const { story } = p;
      if (story === null) throw new Error('test fixture must have story');
      const storyToRemove = story.stories.find((s) => s.id === 'opt_remove');
      ...
      const openTask: Issue = {
        ...mock<Issue>(),
        url: 'https://github.com/acme-labs/ops/issues/103',
        isClosed: false,
        isPr: false,
      };
      const storyObjectMap: StoryObjectMap = new Map([
      ...
      issueRepository.getStoryObjectMap.mockResolvedValue(storyObjectMap);
      issueRepository.clearProjectField.mockRejectedValue(
        new Error('network error'),
      );
    ...
    it('does not call clearProjectField for closed tasks when deleteChildTasks is false', async () => {
      const p = projectWithStoriesToDelete();
      const { story } = p;
      if (story === null) throw new Error('test fixture must have story');
      const storyToRemove = story.stories.find((s) => s.id === 'opt_remove');
      ...
      const closedTask: Issue = {
        ...mock<Issue>(),
        url: 'https://github.com/acme-labs/ops/issues/104',
        isClosed: true,
        isPr: false,
      };
      const storyObjectMap: StoryObjectMap = new Map([
      ...
      expect(issueRepository.clearProjectField).not.toHaveBeenCalled();
    });

    it('closes open tasks when deleteChildTasks is true', async () => {
      const p = projectWithStoriesToDelete();
      const { story } = p;
      if (story === null) throw new Error('test fixture must have story');
      const storyToRemove = story.stories.find((s) => s.id === 'opt_remove');
      ...
      const openTask: Issue = {
        ...mock<Issue>(),
        url: 'https://github.com/acme-labs/ops/issues/105',
        isClosed: false,
        isPr: false,
      };
      const storyObjectMap: StoryObjectMap = new Map([
      ...
      expect(response.statusCode).toBe(200);
      expect(issueRepository.closeIssueByUrl).toHaveBeenCalledWith(
        'https://github.com/acme-labs/ops/issues/105',
        'not_planned',
      );
      expect(issueRepository.clearProjectField).not.toHaveBeenCalled();
    });
    ...
    describe('storyOptionId existence in cached vs. freshly-fetched data (issue #2603)', () => {
      ...
      const existenceCases: {
        ...
        {
          name: 'row 3: storyOptionId present in cached data but absent from fresh data (recently deleted story) -> returns not-found error and deletes nothing',
          cachedIds: ['opt_other', 'opt_target'],
          freshIds: ['opt_other'],
          expectedStatusCode: 400,
          expectDeletionApplied: false,
        },
        {
          name: 'row 4: storyOptionId absent from both cached and fresh data -> returns not-found error',
          cachedIds: ['opt_other'],
      ...
      it.each(existenceCases)(
        ...
        async ({
          ...
          const response = await handleDeleteStory(
            ...
            { pjcode: 'acme', storyOptionId: 'opt_target' },
          );
          expect(response.statusCode).toBe(expectedStatusCode);
          if (expectDeletionApplied) {
            expect(localUpdateStoryList).toHaveBeenCalledTimes(1);
            expect(localUpdateStoryList).toHaveBeenCalledWith(
              expect.anything(),
    ...
    describe('fresh story option resolution and story object map fetch concurrency (issue #2619)', () => {
      it('fetches the fresh story option and the story object map concurrently and issues the story list write only after both reads settle', async () => {
        const p = projectWithStoriesToDelete();
        let resolveGetProject!: (value: Project | null) => void;
        const getProjectPromise = new Promise<Project | null>((resolve) => {
          resolveGetProject = resolve;
        });
        const localGetProject = jest.fn().mockReturnValue(getProjectPromise);
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        const ctx: ConsoleOperationContext = {
          ...deleteStoryContext(p),
          resolveProjectRepository: () => ({
        ...
        const responsePromise = handleDeleteStory(ctx, {
          pjcode: 'acme',
          storyOptionId: 'opt_remove',
        });
    ...
    describe('mutating call must use the freshly-fetched story fieldId, not the stale cached one', () => {
      const buildDeletableProjectWithFieldId = (fieldId: string): Project => ({
        ...projectWithStoriesToDelete(),
        story: {
          name: 'Story',
          fieldId,
      ...
      it('calls updateStoryList with a project whose story.fieldId equals the freshly-fetched fieldId, not the stale cached fieldId', async () => {
        const cachedProject = buildDeletableProjectWithFieldId('PVTSSF_stale');
        const freshProject = buildDeletableProjectWithFieldId('PVTSSF_fresh');
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        const ctx: ConsoleOperationContext = {
          ...deleteStoryContext(cachedProject),
          resolveProjectRepository: () => ({
      ...
      it.each(fallbackCases)(
        ...
        async ({ freshResolution }) => {
          const cachedProject =
            buildDeletableProjectWithFieldId('PVTSSF_stale');
          const freshProjectResolution: Project | null =
            freshResolution === 'null'
              ? null
              : { ...cachedProject, story: null };
  ...
  describe('handleStoryRename', () => {
    ...
    describe('mutating call must use the freshly-fetched story fieldId, not the stale cached one', () => {
      const buildRenamableProjectWithFieldId = (fieldId: string): Project => ({
        ...projectWithStoriesToRename(),
        story: {
          name: 'Story',
          fieldId,
      ...
      it('calls updateStoryList with a project whose story.fieldId equals the freshly-fetched fieldId, not the stale cached fieldId', async () => {
        const cachedProject = buildRenamableProjectWithFieldId('PVTSSF_stale');
        const freshProject = buildRenamableProjectWithFieldId('PVTSSF_fresh');
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        const ctx: ConsoleOperationContext = {
          ...renameStoryContext(cachedProject),
          resolveProjectRepository: () => ({
            updateStoryList: localUpdateStoryList,
            getProject: jest.fn().mockResolvedValue(freshProject),
          }),
        };
        const response = await handleStoryRename(ctx, {
          pjcode: 'acme',
          storyOptionId: 'opt_alpha',
          newName: 'Alpha renamed',
        });
      ...
      it.each(fallbackCases)(
        ...
        async ({ freshResolution }) => {
          const cachedProject =
            buildRenamableProjectWithFieldId('PVTSSF_stale');
          const freshProjectResolution: Project | null =
            freshResolution === 'null'
              ? null
              : { ...cachedProject, story: null };
  ...
  describe('handleStoryUpdateDescription', () => {
    ...
    describe('mutating call must use the freshly-fetched story fieldId, not the stale cached one', () => {
      const buildDescribableProjectWithFieldId = (
        fieldId: string,
      ): Project => ({
        ...projectWithStoriesToUpdateDescription(),
        story: {
      ...
      it('calls updateStoryList with a project whose story.fieldId equals the freshly-fetched fieldId, not the stale cached fieldId', async () => {
        const cachedProject =
          buildDescribableProjectWithFieldId('PVTSSF_stale');
        const freshProject = buildDescribableProjectWithFieldId('PVTSSF_fresh');
        const localUpdateStoryList = jest.fn().mockResolvedValue([]);
        const ctx: ConsoleOperationContext = {
          ...descriptionContext(cachedProject),
          resolveProjectRepository: () => ({
            updateStoryList: localUpdateStoryList,
            getProject: jest.fn().mockResolvedValue(freshProject),
          }),
        };
        const response = await handleStoryUpdateDescription(ctx, {
          pjcode: 'acme',
          storyOptionId: 'opt_alpha',
          description: 'Updated description',
        });
  ...
  describe('handleCreateWorkflowIssue', () => {
    ...
    it('uses empty body when body field is absent', async () => {
      await handleCreateWorkflowIssue(issueRepository, {
        nameWithOwner: 'HiromiShikata/secretary',
        title: 'Fix the workflow pipeline',
      });
    ...
    it('creates issue with the body field directly when body is provided, without double-quoting', async () => {
      const preFormattedBody =
        'https://github.com/owner/repo/issues/1\n\nSource issue title\n\n\n\n\n\n> Line one of the comment.\n> Line two of the comment.';
      const response = await handleCreateWorkflowIssue(issueRepository, {
        nameWithOwner: 'HiromiShikata/secretary',
        title: 'Fix the workflow pipeline',
        body: preFormattedBody,
      });
      expect(response.statusCode).toBe(200);
      expect(issueRepository.createNewIssue).toHaveBeenCalledWith(
        'HiromiShikata',
