...
Options for serveConsole:
  ...
  --dashboardDir <path>                            Directory containing the dashboard file tdpm.txt served at /tdpm.txt (default: /home/hiromi/0_workspaces/workspace1/jsonpub)
...
- Dashboard file (GET): `GET /tdpm.txt` serves the dashboard file `tdpm.txt` byte-for-byte from `--dashboardDir` (default `/home/hiromi/0_workspaces/workspace1/jsonpub`). The file is an HTML fragment, so it is served with `Content-Type: text/html; charset=utf-8`, an explicit `Content-Length` (never chunked), and `Cache-Control: no-store`. It is gated by the same access token, so requests carry `?k=<token>`; a request without a valid token is rejected with HTTP 401. The dashboard file is generated by an external generator, not by this server.
