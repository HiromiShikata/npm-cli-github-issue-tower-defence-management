...
- Otherwise the repository performs an incremental fetch: it queries only items changed on or after the day of the previous `lastFetchedAt` minus one day (the GitHub ProjectV2 `items(query: "updated:>=<YYYY-MM-DD>")` filter is day-granular) and upserts them into the cached list keyed by issue URL, reusing the cached `project` (`cacheUsed` is `true`).
