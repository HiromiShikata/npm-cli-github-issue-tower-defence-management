...
describe('writeInTmuxByHumanData', () => {
  ...
  it('writes the four per-project files and the four index files without producing any v5 files', () => {
    ...
    expect(fs.existsSync(file('demo.v5.json'))).toBe(false);
    expect(fs.existsSync(file('index.v5.json'))).toBe(false);
  });
