...
Options for serveWeb (and its deprecated alias serveConsole):
  ...
  --dashboardDataDir <path>                        Directory containing the dashboard data files (projects/<pjcode>.json, machine-status.json, token-status.json) the server composes the /tdpm.txt fragment from at request time; defaults to the same location the scheduled run emits them to (unset when not configured)
...
The dashboard fragment is served unauthenticated at `GET /tdpm.txt` as `text/html`. The server composes it at request time from the data files the scheduled run emits under `--dashboardDataDir`: one `projects/<pjcode>.json` per project (that project's actionable status counts), `machine-status.json` (mem%, cpu%, loadavg, cycle-minutes), and `token-status.json` (per-token rate-limit utilization, reset countdown, status color, and prep/hum counts). It renders the fixed-width fragment — the host-metrics line, the project grid (header then one row per code in `--dashboardProjectCodes` order with its severity dot), a blank separator line, then one row per token sorted by soonest seven-day reset — wrapped in `<tt>…</tt><br>` with spaces encoded as `&nbsp;`. A project whose file is absent renders placeholder cells; an absent `machine-status.json` renders host-metric placeholders, and an absent `token-status.json` renders no token rows. When `--dashboardDataDir` is not configured the route returns HTTP 404. Only `GET` is accepted on this path.
