...
test('restores a todo-by-human item when a stale done overlay entry predates the snapshot generatedAt and the item is still present (self-heal)', async ({
  page,
}) => {
  await page.goto(harness.appRootUrl);
...
test('create-task dialog submits agent, body, and file attachment correctly', async ({
  ...
  const initialCommentCount = harness.commentCalls.length;
  ...
  await expect
    .poll(() => harness.commentCalls.length, { timeout: 10000 })
    .toBe(initialCommentCount + 1);
  expect(harness.commentCalls.at(-1)?.body).toContain('fixture-attachment.png');
});
...
test('shows fleet task create button and creates issue via dialog when title is submitted', async ({
  browser,
}) => {
