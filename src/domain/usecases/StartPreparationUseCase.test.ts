...
describe('StartPreparationUseCase', () => {
  ...
  describe('writes resolved defaultAgentName to the Agent field when the issue has no agent configured', () => {
    ...
    it('sets the Agent field to the matching option when the project has a matching agent option and the issue has no agent configured', async () => {
    ...
    it('writes the resolved agent name to the Agent field before spawning the worker', async () => {
      ...
      expect(
        mockIssueRepository.setIssueAgentField.mock.invocationCallOrder[0],
      ).toBeLessThan(
  ...
  it('should pass --branch to aw command when issue has an existing linked PR', async () => {
    ...
    const relatedPrIssue = createMockIssue({
      ...
      isPr: true,
      isClosed: false,
      closingIssueReferenceUrls: ['url1'],
    });
    mockProjectRepository.getByUrl.mockResolvedValue(mockProject);
    mockIssueRepository.getStoryObjectMap.mockResolvedValue(
      createMockStoryObjectMap([...awaitingIssues, relatedPrIssue]),
    );
  ...
  it('should exclude an Awaiting Workspace board item from the spawn loop when the item itself is a PR (isPr: true), even though its own URL is a pull request URL', async () => {
    const awaitingIssues: Issue[] = [
      createMockIssue({
  ...
  it('should skip and not call wrapper when issue has one related open PR whose branch name has shell-unsafe characters', async () => {
    const awaitingIssues: Issue[] = [
      createMockIssue({
    ...
    const prWithUnsafeBranch: RelatedPullRequest = {
      url: 'https://github.com/user/repo/pull/42',
    ...
    const prIssueWithUnsafeBranch = createMockIssue({
      url: prWithUnsafeBranch.url,
      number: 42,
      isPr: true,
      isClosed: false,
      closingIssueReferenceUrls: ['https://github.com/user/repo/issues/999'],
    });
    mockProjectRepository.getByUrl.mockResolvedValue(mockProject);
    mockIssueRepository.getStoryObjectMap.mockResolvedValue(
      createMockStoryObjectMap([...awaitingIssues, prIssueWithUnsafeBranch]),
    );
    mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([
      prWithUnsafeBranch,
    ]);
  ...
  it('should skip and not call wrapper when issue has one related open PR with null branchName', async () => {
    ...
    mockProjectRepository.getByUrl.mockResolvedValue(mockProject);
    mockIssueRepository.getStoryObjectMap.mockResolvedValue(
      createMockStoryObjectMap([...awaitingIssues, prIssueWithNullBranch]),
    );
    mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([
      prWithNullBranch,
    ]);
  ...
  it('should pick the soonest-7-day-reset token first even when existing Preparation issues exist (regression for issue 748)', async () => {
    const preparationIssues: Issue[] = Array.from({ length: 3 }, (_, i) =>
      createMockIssue({
        url: `https://github.com/user/repo/issues/${i + 1}`,
        title: `Preparing Issue ${i + 1}`,
        labels: [],
        status: 'Preparation',
        number: i + 1,
        itemId: `item-${i + 1}`,
    ...
    mockProjectRepository.getByUrl.mockResolvedValue(mockProject);
    mockIssueRepository.getStoryObjectMap.mockResolvedValue(
      createMockStoryObjectMap([...preparationIssues, awaitingIssue]),
    );
    mockLocalCommandRunner.runCommand.mockResolvedValue({
      stdout: '',
      stderr: '',
  ...
  it('should select a token whose legacy per-model limit is rejected but the unified seven_day key is absent', async () => {
    ...
    mockClaudeTokenUsageRepository.getAvailableTokenUsages.mockResolvedValue([
      {
        name: 'token-with-opus-rejected',
        token: 'token-with-opus-rejected',
        fiveHourUtilization: 0.1,
        sevenDayUtilization: 0,
        blocked: false,
        rejected: false,
    ...
    expect(mockLocalCommandRunner.runCommand.mock.calls).toHaveLength(1);
    expect(mockLocalCommandRunner.runCommand.mock.calls[0][2]).toMatchObject({
      env: { CLAUDE_CODE_OAUTH_TOKEN: 'token-with-opus-rejected' },
    });
  });
  ...
  it('only prefetches branch sources for candidates within the free preparation slots', async () => {
    ...
    const prIssuesEnablingBoardCacheGuard = Array.from({ length: 5 }, (_, i) =>
      createMockIssue({
        ...
        isClosed: false,
        closingIssueReferenceUrls: [
          `https://github.com/user/repo/issues/${i + 1}`,
        ],
      }),
    );
    mockProjectRepository.getByUrl.mockResolvedValue(mockProject);
...
describe('StartPreparationUseCase.buildRotationOrder', () => {
  ...
  it('selects a token whose overall rejected status is true but the unified seven_day weekly window is not rejected', () => {
    const nowEpochSeconds = Math.floor(Date.now() / 1000);
    const tokenUsages: ClaudeTokenUsage[] = [
      {
        name: 'overall-rejected-seven-day-ok',
        token: 'sk-ant-overall-rejected',
    ...
    expect(result).toHaveLength(1);
    expect(result[0].name).toBe('overall-rejected-seven-day-ok');
    expect(result[0].rejected).toBe(false);
    expect(result[0].thresholdExcluded).toBe(false);
    expect(result[0].cooldownExcluded).toBe(false);
  });
...
describe('StartPreparationUseCase.run board-cache PR guard', () => {
  it('does not call findRelatedOpenPRs for candidates when the board cache shows no related open PRs', async () => {
    const mockProject = createMockProject();
    const awaitingIssue = createMockIssue({
      url: 'https://github.com/user/repo/issues/1',
      status: 'Awaiting Workspace',
  ...
  it('calls findRelatedOpenPRs for a Story-unset awaiting-workspace candidate whose Story-unset open PR is only visible via getAllOpened', async () => {
    const mockProject = createMockProject();
    const awaitingIssue = createMockIssue({
      url: 'https://github.com/user/repo/issues/1',
      status: 'Awaiting Workspace',
...
describe('StartPreparationUseCase.fetchSpawnCandidateBranchSources', () => {
  ...
  it('skips findRelatedOpenPRs and returns empty relatedOpenPullRequests for issue URLs not in the known open PRs set', async () => {
    ...
    expect(findRelatedOpenPRs).not.toHaveBeenCalled();
    expect(branchSources.get(issueUrl)?.relatedOpenPullRequests).toEqual([]);
    expect(branchSources.get(issueUrl)?.openPullRequest).toBeNull();
  });

  it('calls findRelatedOpenPRs only for issue URLs in the known open PRs set', async () => {
    const issueWithPr = 'https://github.com/owner/repo/issues/1';
    const issueWithoutPr = 'https://github.com/owner/repo/issues/2';
    const findRelatedOpenPRs = jest.fn().mockResolvedValue([]);
    const useCase = buildUseCase({ findRelatedOpenPRs });

    await useCase.fetchSpawnCandidateBranchSources(
      [issueWithPr, issueWithoutPr],
      new Set([issueWithPr]),
    );

    expect(findRelatedOpenPRs).toHaveBeenCalledTimes(1);
    expect(findRelatedOpenPRs).toHaveBeenCalledWith(issueWithPr);
    expect(findRelatedOpenPRs).not.toHaveBeenCalledWith(issueWithoutPr);
  });
});
