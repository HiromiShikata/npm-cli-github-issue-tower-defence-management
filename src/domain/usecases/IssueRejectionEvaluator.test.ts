...
describe('IssueRejectionEvaluator', () => {
  ...
  describe('evaluate', () => {
    ...
    describe('pullRequestRequired: false in issue body', () => {
      it('should not reject and not call findRelatedOpenPRs when body ends with JSON block containing pullRequestRequired: false', async () => {
      ...
      it('should not reject even with trailing whitespace after the closing JSON block', async () => {
        const body = '```json\n{"pullRequestRequired": false}\n```\n  ';
      ...
      it('should still require PR when body has pullRequestRequired: true', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
      ...
      it('should still require PR when body is null', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
      ...
      it('should still require PR when body is undefined', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
      ...
      it('should still require PR when JSON block with pullRequestRequired: false is not at end of body', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
        const body =
          '```json\n{"pullRequestRequired": false}\n```\n\nsome text after';
      ...
      it('should still require PR when body has invalid JSON in code block', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
        const body = '```json\n{invalid json}\n```';
      ...
      it('should still require PR when body has pullRequestRequired not set to false', async () => {
        mockIssueRepository.findRelatedOpenPRs.mockResolvedValue([]);
        const body = '```json\n{"someOtherField": true}\n```';
