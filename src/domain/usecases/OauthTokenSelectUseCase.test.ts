import {
  CL_SCRIPT_FALLBACK_SEVEN_DAY_MIN_FREE_RATIO,
...
describe('selectFallback', () => {
  ...
  it('selects the candidate with the highest 5h free ratio when all fail CL script thresholds', () => {
    ...
    expect(result.selected?.name).toBe('high5h');
    expect(result.metrics.every((m) => m.drawWeight === 0)).toBe(true);
  });

  it('returns null when all candidates have < 3% 7d free', () => {
    const result = useCase.selectFallback(
      [
        candidate('depleted', snapshot({ sevenDayUtilization: 0.98 })),
        candidate('alsoDepleted', snapshot({ sevenDayUtilization: 0.99 })),
      ],
      NOW,
    );

    expect(result.selected).toBeNull();
    const depleted = result.metrics.find((m) => m.name === 'depleted');
    expect(depleted?.exclusionReason).toContain('fallback requires >= 3%');
  });

  it('excludes subscription-disabled candidates in the fallback', () => {
    const result = useCase.selectFallback(
      [
        candidate('disabled', snapshot({}), true),
        candidate('active', snapshot({}), false),
  ...
  it('excludes unified-rejected candidates in the fallback', () => {
    const result = useCase.selectFallback(
      [
        candidate('rejected', snapshot({}), false, true),
        candidate('active', snapshot({}), false, false),
  ...
  it('excludes fable-rejected candidates in the fallback', () => {
    const result = useCase.selectFallback(
      [
        candidate('fable-out', snapshot({}), false, false, true),
        candidate('active', snapshot({}), false, false, false),
