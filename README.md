...
Commands:
  ...
  writeAgentHeartbeat [options]         Write a heartbeat for an issue being actively worked by this agent, so the orphan-revert sweep does not reclaim it
...
Options for writeAgentHeartbeat:
  --issueUrl <url>                                 GitHub issue URL that the calling agent is working on (required)
  --heartbeatDir <path>                            Directory for heartbeat files (default: /tmp/tdpm-agent-heartbeats)
...
The `writeAgentHeartbeat` sub-command writes a heartbeat timestamp for the given issue URL to a file in `--heartbeatDir` (default: `/tmp/tdpm-agent-heartbeats`). The heartbeat file is named by replacing non-alphanumeric characters in the issue URL (after stripping the scheme) with `-`, so two agents working different issues never collide. The command writes nothing to stdout on success.

The periodic orphan-revert sweep (`RevertOrphanedInTmuxByAgentIssueUseCase`) decides whether an `In Tmux by agent` issue is abandoned using evidence it collects itself: (1) a live tmux session named after the issue URL indicates a subagent is working — the sweep refreshes the heartbeat automatically on every cycle; (2) the issue URL appearing in a running process command line has the same effect; (3) when neither signal is present the sweep records an orphan-candidate timestamp the first time it notices the issue and gives the agent a grace period of `minOrphanAgeSeconds` (default: 604800 s, i.e. 7 days) before reconsidering; (4) after the grace period a fresh heartbeat written by `writeAgentHeartbeat` (no less often than every `minOrphanAgeSeconds / 2` seconds, i.e. once every 3.5 days by default) still prevents reclaim. An issue with no named session, no process match, an orphan-candidate older than the grace period, and no fresh heartbeat is reverted to `Awaiting Workspace`. The heartbeat is optional for workspace-leader sessions: without it an issue is reclaimed after one grace period beyond when the sweep first notices it as a potential orphan. The 7-day default was chosen from observed real-work duration on this machine: the longest live session measured was 45.9 hours, so any threshold below roughly 2 days would reclaim issues that agents are still actively working; issues truly abandoned had their status last written 40–56 days earlier, leaving ample margin between the two populations.
