...
subAgentReminderEscalationSeconds?: number # Optional: Seconds that must elapse before a reminder triggered only by the sub-process section is re-sent to the same session while the triggering sub-process state is unchanged (every currently flagged sub-process label was already in the last send; an idle-flagged sub-process additionally counts as changed when it has produced new output since then, while a sub-process flagged only for running long is unchanged regardless of output because its running time never resets). A changed sub-process state re-notifies immediately, and main-session stall notifications are never affected. Default 1800
