...
describe('OauthAPIClaudeRepository', () => {
  ...
  describe('isClaudeAvailable with claudeCodeOauthTokenListJsonPath', () => {
    ...
    it('should set selectedToken after isClaudeAvailable selects a token', async () => {
      mockFs.existsSync.mockImplementation((p) => p === tokenListPath);
      mockFs.readFileSync.mockReturnValue(
      ...
      expect(repo.getSelectedToken()).toBe('token-under-threshold');
    });
  });
  ...
  describe('getUsage with claudeCodeOauthTokenListJsonPath', () => {
    ...
    it('should select a token from the list and expose it via getSelectedToken', async () => {
      mockFs.existsSync.mockImplementation((p) => p === tokenListPath);
      ...
      expect(repo.getSelectedToken()).toBe('test-token');
    });
  });
