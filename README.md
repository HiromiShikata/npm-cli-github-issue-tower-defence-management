...
readOnlyGithubTokens?: string[] # Optional: Array of GitHub tokens used exclusively for read-only GitHub API calls made by the Console HTTP server (serveWeb). When set and non-empty, the Console cycles through these tokens when a call returns a rate-limit error (HTTP 403 with rate-limit signals), trying each token in order until one succeeds. Write operations (comments, status updates, etc.) always use the default project token regardless of this setting. When unset or empty, the Console uses the per-project token resolution from consoleGithubTokens.
...
readGhTokens?: string[] # Optional: Array of GitHub personal access tokens with read-only scope. When set, all GET (read-only) GitHub API calls — REST reads via RestIssueRepository and RestProjectRepository, and non-mutation GraphQL queries via GraphqlProjectRepository — rotate across these tokens in round-robin order. Mutation calls (POST, PATCH, PUT, DELETE, and GraphQL mutations) always use the manager token from GH_TOKEN regardless of this setting. When omitted or empty, the manager token is used for all calls, preserving the existing behavior.
```
