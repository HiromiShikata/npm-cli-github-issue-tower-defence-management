...
startPreparation?: # Optional: Enable automatic issue preparation workflow
  ...
  allowedLlmModelNames?: string[] | null # Optional: List of permitted model names for the llm-model: label. When set, a llm-model: label whose value is not in this list causes the issue to be skipped with an error log naming the rejected value and the issue URL. When absent, any llm-model: label value is accepted with a warning log (backward compatible)
