...
describe('StartPreparationUseCase', () => {
  ...
  describe('allowedLlmModelNames allowlist', () => {
    const baseRunParams = {
      projectUrl: 'https://github.com/user/repo',
      defaultAgentName: 'agent1',
      defaultLlmModelName: 'claude-opus',
    ...
    const arrangeIssueWithLabel = (label: string): void => {
    ...
    it('processes the issue normally when the llm-model: label value is in allowedLlmModelNames', async () => {
      arrangeIssueWithLabel('llm-model:claude-sonnet-4-6');
      await useCase.run({
        ...baseRunParams,
        allowedLlmModelNames: ['claude-sonnet-4-6', 'claude-opus-5'],
      });
      expect(mockLocalCommandRunner.runCommand.mock.calls).toHaveLength(1);
      expect(mockLocalCommandRunner.runCommand.mock.calls[0][1]).toContain(
        'claude-sonnet-4-6',
    ...
    it('skips the issue and logs an error when the llm-model: label value is not in allowedLlmModelNames', async () => {
      arrangeIssueWithLabel('llm-model:arbitrary-value');
      const consoleErrorSpy = jest
        .spyOn(console, 'error')
        .mockImplementation(() => {});
      await useCase.run({
        ...baseRunParams,
        allowedLlmModelNames: ['claude-sonnet-4-6'],
      });
      expect(mockLocalCommandRunner.runCommand.mock.calls).toHaveLength(0);
      const errorCalls = consoleErrorSpy.mock.calls.filter((call) =>
        String(call[0]).includes('arbitrary-value'),
      );
    ...
    it('processes the issue with a warning when allowedLlmModelNames is null and a llm-model: label is present', async () => {
      arrangeIssueWithLabel('llm-model:arbitrary-value');
      const consoleWarnSpy = jest
        .spyOn(console, 'warn')
        .mockImplementation(() => {});
