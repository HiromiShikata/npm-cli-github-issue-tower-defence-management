...
class LocalProcessLiveSessionProcessSnapshotProvider {
    constructor(localCommandRunner, environReader) {
        ...
        this.listPanePids = async (sessionName) => {
            const { stdout, exitCode } = await this.localCommandRunner.runCommand('tmux', ['list-panes', '-t', sessionName, '-F', '#{pane_pid}']);
            if (exitCode !== 0) {
                return [];
            }
            return stdout
        ...
        this.listProcesses = async () => {
            const { stdout, exitCode } = await this.localCommandRunner.runCommand('ps', ['-eo', 'pid=,ppid=,args=']);
            if (exitCode !== 0) {
                return [];
            }
            const processes = [];
...
exports.LocalProcessLiveSessionProcessSnapshotProvider = LocalProcessLiveSessionProcessSnapshotProvider;
//# sourceMappingURL=LocalProcessLiveSessionProcessSnapshotProvider.js.map